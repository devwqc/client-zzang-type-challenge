...
/*
  🤔 접근
    1. 제네릭 K 를 keyof T 로 타입 제한
      - error 케이스 대응

    2. 제네릭 K 에 기본값 keyof T 를 넣어 옵셔널 처리
      - 테스트 케이스 1번 대응
      - K 가 주어지지 않으면 T 의 모든 키는 readonly 처리 필요
        - K 를 Mapped Types 로 readonly 처리하고, Intersection Type 으로 K 에 포함되지 않는 프로퍼티를 Mapped Types 로 처리

...
/**
접근
1. type IsUnion<T, C = T> = T extends C ? false : true
=> T extends C는 항상 참
비교하려는 C를 개별로 쪼개야 함

2. type IsUnion<T, C = T> = [T] extends [C] ? false : true
=> []로 감싸면 C, T는 안 쪼개짐

3. type IsUnion<T, C = T> = T extends C ? [C] extends [T] ? false : true : never
=> never 통과가 안된다!
=> [T] extends [never] 추가하기
 */
